...
parser.add_argument('--N', '-N', type=int, default=10, help='Number of nearest neighbors to find')
parser.add_argument('--M', '-M', type=int, default=400, help='Grid size for Poisson reconstruction')
...
# kNN graph
knnGraph = kneighbors_graph(points, n_neighbors=N, mode='connectivity', include_self=False, n_jobs=-1) # n_jobs=-1 to use all processors
knnGraph = knnGraph.maximum(knnGraph.T)
knnGraph = knnGraph.tocsr()

# Normals
normals = np.array([get_normal(point, points, kdtree, N) for point in points])
...
sigma = estimate_sigma(points, N)

print("Starting Poisson surface reconstruction...")
start_poisson = time.time()
x,y,z,chi = poisson_solver.solve_poisson(points, oriented_normals, sigma=sigma, N=M)
...
with open("plots/timings.txt", "a") as f:
    f.write(f"{M};{tnorm:.6f};{tpoisson:.6f};{tsave:.6f};{time.time()-start_all:.26f}\n")
